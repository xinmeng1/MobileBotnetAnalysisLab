...
destIpToDomainOutput1= "IPdomainNormal.csv"
destIpToDomainOutput2= "IPdomainInfected.csv"
targetdestIpToDomainOutput1 = OutputDataPath+destIpToDomainOutput1;
targetdestIpToDomainOutput2 = OutputDataPath+destIpToDomainOutput2;
...
def processSampleDataDir(dir,file):
    ...
    ## 处理IP及其域名访问
    normal_dest_ip_list = [[0 for col in range(3)] for row in range(0)];
    infect_dest_ip_list = [[0 for col in range(3)] for row in range(0)];
    temp_dest_ip =['0','0','0'];
    normal_dest_ip_list_Num = 0
    infect_dest_ip_list_Num = 0
    ...
    with open(targetdestIpToDomainOutput1, 'wSb+') as csvfile:
        spamwriter = csv.writer(csvfile, dialect='excel')
        spamwriter.writerow(['IP dest','Lable','Domain'])
    with open(targetdestIpToDomainOutput2, 'wSb+') as csvfile:
        spamwriter = csv.writer(csvfile, dialect='excel')
        spamwriter.writerow(['IP dest','Lable','Domain'])
                ...
                for txtline in open(SampleDataPath+line+"\\"+li):
                    ...
                    if line == '0':
                        ...
                        if len(txtarray[0])<=15:
                            ...
                            ####处理ip地址,将ip地址加入到normal_dest_ip_list中,同样的ip不重复加入.
                            status = 0;
                            for i in normal_dest_ip_list:
                                if(i[0] == txtarray[1]):
                                    status = 1;
                                    break;
                            if(status == 0):
                                ###不重复,则需要加入到 normal_dest_ip_list
                                #temp_dest_ip.insert(0,txtarray[1]);
                                temp_dest_ip[0] = txtarray[1];
                                #temp_dest_ip.insert(1,'nromal');
                                temp_dest_ip[1] = 'nromal';
                                try:
                                    URLresult = socket.gethostbyaddr(txtarray[1]);
                                except socket.herror,e:
                                    URLresult = "Can not Find";
                                #exit()
                                #temp_dest_ip.insert(2,URLresult);
                                temp_dest_ip[2] = URLresult;
                                print temp_dest_ip;
                                normal_dest_ip_list = [temp_dest_ip]+normal_dest_ip_list;
                                normal_dest_ip_list_Num = normal_dest_ip_list_Num+1;
                            ...
                            with open(targetdestIpToDomainOutput1, 'ab+') as csvfile:
                                spamwriter = csv.writer(csvfile, dialect='excel');
                                spamwriter.writerow(normal_dest_ip_list);
                    ...
                    if line == '1':
                        if len(txtarray[0])<=15:
                            ...
                            if(status == 0):
                                ...
                                for i in normal_dest_ip_list:
                                    if( i[0] == txtarray[1]):
                                        ...
                                        break;
                                if(status_infecte == 1):
                                    #temp_dest_ip.insert(1,'normal');
                                    temp_dest_ip[1] = 'normal'
                                else:
                                    #temp_dest_ip.insert(1,'infected');
                                    temp_dest_ip[1] = 'infected'
                                ...
                                temp_dest_ip[2] = URLresult;
                                print temp_dest_ip;
                                infect_dest_ip_list = [temp_dest_ip]+infect_dest_ip_list;
                                infect_dest_ip_list_Num = infect_dest_ip_list_Num+1;
                            ...
                            with open(targetdestIpToDomainOutput2, 'ab+') as csvfile:
                                spamwriter = csv.writer(csvfile, dialect='excel');
                                spamwriter.writerow(infect_dest_ip_list);                   
